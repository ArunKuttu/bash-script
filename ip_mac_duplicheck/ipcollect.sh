...
awk '/fixed-address/ {for (i=1; i<=NF; i++) if ($i ~ /fixed-address/) {gsub(/;/, "", $(i+1)); print $(i+1)}}' /etc/dhcp/include/dhcpd.conf_static_1 ##make sure to change the file like in your DHCP config file > ipsout1.txt
